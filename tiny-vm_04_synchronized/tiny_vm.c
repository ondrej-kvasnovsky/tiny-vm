...
VM* create_vm() {
    ...
    pthread_mutex_init(&vm->heap_mngt_lock, NULL);
...
void destroy_vm(VM* vm) {
    ...
    free(vm->heap);
    pthread_mutex_destroy(&vm->heap_mngt_lock);
...
Variable* get_shared_variable(ThreadContext* thread, const char* name) {
    ...
    pthread_mutex_lock(&thread->vm->heap_mngt_lock);
    if (thread->vm->heap_size < thread->vm->heap_capacity) {
        ...
        pthread_mutex_unlock(&thread->vm->heap_mngt_lock);
